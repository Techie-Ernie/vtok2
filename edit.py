...
def get_predictions(video_path, debug=False):
    ...
    if success:
        ...
        if debug:
            ...
            x, y, width, height = result['predictions'][0]['x'], result['predictions'][0]['y'], result['predictions'][0]['width'], result['predictions'][0]['height']
...
def edit_video(predictions, video_path, output_dir='.'):
    ...
    final_clip.write_videofile(f"{output_dir}/{os.path.splitext(video_path)[0]}_final.mp4")
