...
def extract_images(video_path, output_dir='images/', frame_interval=540, debug=False):
    ...
    while True:
        ...
        if frame_number % frame_interval == 0:
            ...
            if count == 2:  # Check if there are 2 elements e.g. ['0', '0']
                ...
                if self_score.isdigit() and enemy_score.isdigit(): # Check both are valid integers - can do some additional checking here to see if numbers make sense in case of inaccurate recognition
                    if int(self_score) >= prev_self_score or int(enemy_score) >= prev_enemy_score:
    ...
    print(f'Extraction completed in {end - start} seconds') # Time taken for prog to run 
    cap.release()
    return score_dict
